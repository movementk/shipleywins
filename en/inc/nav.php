<!-- GNB Nav -->
<nav id="gnb-nav">
    <div class="nav-backdrop"></div>
    <div class="nav-container">
        <button type="button" class="btn btn-close">
            <img src="/en/assets/images/main/btn_icon_close.png" alt="네비 닫기">
        </button>
        <ul>
            <li>
                <a href="#">Company<span></span></a>
                <ul>
                    <li><a href="/en/company/greeting.php">CEO's Message</a></li>
                    <li><a href="/en/company/introduce.php">Company introduction</a></li>
                    <li><a href="/en/company/history.php">Major History of Shipley Korea</a></li>
                    <li><a href="/en/company/partner.php">Client company</a></li>
                    <li><a href="/en/company/succession_case.php">Customer message</a></li>
                </ul>
            </li>
            <li>
                <a href="#">Overseas order consulting<span></span></a>
                <ul>
                    <li><a href="/en/abroad_consulting/proposal.php">Consulting for development of international proposal</a></li>
                    <li><a href="/en/abroad_consulting/competition_pt.php">overseas orders PT consulting</a></li>
                    <li><a href="/en/abroad_consulting/strategy.php">International specialization</a></li>
                </ul>
            </li>
            <li>
                <a href="#">Domestic order consulting<span></span></a>
                <ul>
                    <li><a href="/en/internal_consulting/proposal.php">Consultation of Proposal Development</a></li>
                    <li><a href="/en/internal_consulting/competition_pt.php">Bidding / Pitching Consultation</a></li>
                    <li><a href="/en/internal_consulting/onepoint.php">One Point Consulting</a></li>
                    <li><a href="/en/internal_consulting/orders_process.php">Consulting to construct an order process</a></li>
                    <li><a href="/en/internal_consulting/business_consulting.php">Sales consulting</a></li>
                    <li><a href="/en/internal_consulting/business_diagnosis.php">Diagnosis consulting</a></li>
                </ul>
            </li>
            <!--li>
                <a href="#">디자인 컨설팅<span></span></a>
                <ul>
                    <li><a href="/en/design_consulting/about_us.php">About us</a></li>
                    <li><a href="/en/design_consulting/definition.php">디자인 컨설팅이란</a></li>
                    <li><a href="/en/design_consulting/seminar.php">디자인 세미나</a></li>
                    <li><a href="/en/design_consulting/portfolio.php"> Portfolio</a></li>
                </ul>
            </li-->
            <li>
                <a href="#">Training<span></span></a>
                <ul>
                    <li><a href="/en/expert_training/sales.php">B2B Sales Training</a></li>
                    <li><a href="/en/expert_training/strategy.php">Training for Writing Strategy Statements and Proposals</a></li>
                    <li><a href="/en/expert_training/proposal_pt.php">Proposal Presentation Training</a></li>
                    <li><a href="/en/expert_training/special_lecture.php">Special Lecture for Winning a Business</a></li>
                </ul>
            </li>
            <li>
                <a href="#">PR<span></span></a>
                <ul>
                    <li><a href="/en/press/news_list.php">Shipley News</a></li>
                    <li><a href="/en/press/press_list.php">Press Release</a></li>
                </ul>
            </li>
            <li>
                <a href="#">Contact Us<span></span></a>
                <ul>
                    <li><a href="/en/contact/faq_list.php">FAQ</a></li>
                    <!--li><a href="#">인재 채용</a></li-->
                    <li><a href="/en/contact/location.php">찾아오시는 길</a></li>
                </ul>
            </li>
        </ul>
        <!--<div class="seminar-btn">
            <a href="#" class="btn btn-white" role="button">공개세미나 신청하기</a>
        </div>-->
        <div class="sns">
            <ul>
                <li><a href="https://www.facebook.com/shipleykorea" target="_blank">f</a></li>
                <li><a href="http://blog.naver.com/shipleykorea" target="_blank">b</a></li>
            </ul>
        </div>
    </div>
</nav>
<!-- //GNB Nav -->

<!-- Client Nav -->
<nav id="client-nav">
    <div class="nav-backdrop"></div>
    <div class="nav-container">
        <button type="button" class="btn btn-close">
            <img src="/en/assets/images/main/btn_icon_close.png" alt="네비 닫기">
        </button>
        <div class="client-info">
            <div class="container-fluid">
                <h2>Client Company</h2>
                <!-- Nav tabs -->
                <ul class="nav-list nav nav-tabs" role="tablist">
                    <li role="presentation" class="active">
                        <a href="#aviation" aria-controls="aviation" role="tab" data-toggle="tab">
                            방위/항공<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#si-it" aria-controls="si-it" role="tab" data-toggle="tab">
                            SI/IT<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#telecommunications" aria-controls="telecommunications" role="tab" data-toggle="tab">
                            통신<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#build-manufacture" aria-controls="build-manufacture" role="tab" data-toggle="tab">
                            제조/건설<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#service-consulting" aria-controls="service-consulting" role="tab" data-toggle="tab">
                            서비스/컨설팅<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#financial" aria-controls="financial" role="tab" data-toggle="tab">
                            금융<span></span>
                        </a>
                    </li>
                    <li role="presentation">
                        <a href="#logistics" aria-controls="logistics" role="tab" data-toggle="tab">
                            물류<span></span>
                        </a>
                    </li>
                </ul>

                <!-- Tab panes -->
                <div class="client-list tab-content">
                    <div role="tabpanel" class="tab-pane active" id="aviation">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="si-it">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="telecommunications">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="build-manufacture">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="service-consulting">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="financial">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="logistics">
                        <ul class="row">
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img07.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img08.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img12.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img09.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img10.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img11.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img03.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img04.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img05.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img06.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img13.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img14.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img01.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                            <li class="col-xs-6 col-sm-4">
                                <a href="#" target="_blank">
                                    <img src="/en/assets/images/main/client_logo_img02.gif" class="img-responsive" alt="">
                                </a>
                            </li>
                        </ul>
                        <p class="success-btn">
                            <a href="/en/company/succession_case.php" class="btn btn-blue btn-lg" role="button">Customer message</a>
                        </p>
                    </div>
                </div>    
            </div>
        </div>
    </div>               
</nav>
<!-- //Client Nav -->